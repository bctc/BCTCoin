...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BctCoin")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BctCoin unit input in lnReqAmount, we generate the URI with BctCoin as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BctCoin, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
