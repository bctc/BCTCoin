...
// Copyright (c) 2014 The BCTCoin developers
...
uint256 hashGenesisBlock("0x000000fe388fc691fb3e68234688d25069177b8f7337e8797e75bf92f9262aaa");
...
const string strMessageMagic = "BCTCoin Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 1500 BCT if made of 50 BCT inputs)
            if (nBytes < 100)
                nMinFee = 0;
        }
        else
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    if      (nHeight <=10 )     { nSubsidy = 250000 * COIN; }       /* For Premine */
	else if (nHeight <= 2000)   { nSubsidy = 2 * COIN; }			/* Control Flow */
	else if (nHeight <= 100000) { nSubsidy = 9 * COIN; }
	else if (nHeight <= 200000) { nSubsidy = 7 * COIN; }
	else if (nHeight <= 300000) { nSubsidy = 6 * COIN; } 
	else if (nHeight <= 400000) { nSubsidy = 3 * COIN; } 
	else if (nHeight <= 414048) { nSubsidy = 1 * COIN; } 
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
        /* current difficulty formula, BCTCoin - kimoto gravity well */
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000000037fd834f97fa06e7442a8549e12d66abbea82f6d0931c901fffe08937");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Welcome to Bit Coin Talk Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396027070;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3494869 ;

        if (fTestNet)
        {
            block.nTime    = 1396009776;
            block.nNonce   = 39989459;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd29f38005fbffe3a9319003ea6f6b1fe0331fefcb0d0b90dcd7e3ae51719c049"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BCTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BCTCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("BCTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BCTCoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running BCTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("BCTCoinMiner terminated\n");
        throw;
    }
}
